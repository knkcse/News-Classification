...
model = RandomForestClassifier(n_estimators=len(news_df)/2, max_depth=len(news_df)/2,n_jobs=5,random_state=200)
#print model
...
for i in range(0,len(te['news'])):
	...
	pred= pred.append({'news': te['news'][i],'category':y_pred[i]}, ignore_index=True)
print pred
